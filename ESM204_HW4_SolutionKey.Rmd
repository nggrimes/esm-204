...
damages_PV <- lapply(1:length(r), function(x) 
  bind_cols(r= r[x], 
            PVdiff_perton = warming_damages$damage_diff_perton/(1+r[x])^(warming_damages$year-2022),
...
\noindent The red line in Figure 3 shows the SCC of around $\$71$ at the Ramsey rule discount rate of 2.1\%.
...
policy_damages <- damages_PV %>%
  ...
  mutate(damage_A1=(b_1*warm_A1 + b_2*warm_A1^2)/((1+0.02)^(year-2022))) %>%
  mutate(damage_A2=(b_1*warm_A2 + b_2*warm_A2^2)/((1+0.02)^(year-2022))) %>%
  mutate(damage_B=(b_1*warm_B + b_2*warm_B^2)/((1+0.02)^(year-2022))) 
...
cost_scenario = tibble(X=x, cost_b = 787+x, cost_a=3023)

plot_4 <- ggplot(cost_scenario, aes(X, y=cost_b)) +
  geom_line() +
  geom_hline(yintercept = 3023, color = 'mistyrose4')  +
  theme_cowplot() +
